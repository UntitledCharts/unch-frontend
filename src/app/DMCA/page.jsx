'use client';
import { useLanguage } from '../../contexts/LanguageContext';
import Link from 'next/link';
import { FileWarning, MessageSquare, Edit3, ArrowLeft } from 'lucide-react';
import './page.css';

export default function DMCAPage() {
    const { t } = useLanguage();

    return (
        <main className="dmca-page animate-fade-in">
            <div className="dmca-bg-blur" />

            <div className="back-btn-container" style={{ position: 'relative', zIndex: 10, maxWidth: '1000px', margin: '0 auto', paddingTop: '100px', paddingBottom: '20px', paddingLeft: '20px' }}>
                <Link href="/" className="back-btn" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '8px', color: 'var(--foreground)', opacity: 0.7 }}>
                    <ArrowLeft size={20} />
                    {t('dmca.backToHome')}
                </Link>
            </div>

            <div className="dmca-container">
                <div className="dmca-header">
                    <div className="icon-container">
                        <FileWarning size={32} />
                    </div>
                    <h1 className="dmca-title">{t('dmca.title')}</h1>
                    <p className="dmca-subtitle">{t('dmca.subtitle')}</p>
                </div>

                <div className="tutorial-steps">
                    {}
                    <div className="step-card animate-slide-up" style={{ animationDelay: '0.1s' }}>
                        <div className="step-number">1</div>
                        <div className="step-content">
                            <h2>{t('dmca.step1Title')}</h2>
                            <p>{t('dmca.step1Desc')}</p>
                            <a href="https://discord.com/invite/mH3xWPPdEY" target="_blank" rel="noreferrer" className="discord-btn">
                                <svg width="18" height="18" viewBox="0 0 127.14 96.36" fill="currentColor">
                                    <path d="M107.7,8.07A105.15,105.15,0,0,0,81.47,0a72.06,72.06,0,0,0-3.36,6.83A97.68,97.68,0,0,0,49,6.83,72.37,72.37,0,0,0,45.64,0,105.89,105.89,0,0,0,19.39,8.09C2.79,32.65-1.71,56.6.54,80.21h0A105.73,105.73,0,0,0,32.71,96.36,77.7,77.7,0,0,0,39.6,85.25a68.42,68.42,0,0,1-10.85-5.18c.91-.66,1.8-1.34,2.66-2a75.57,75.57,0,0,0,64.32,0c.87.71,1.76,1.39,2.66,2a68.68,68.68,0,0,1-10.87,5.19,77.67,77.67,0,0,0,6.89,11.1A105.25,105.25,0,0,0,126.6,80.22h0C129.24,52.84,122.09,29.11,107.7,8.07ZM42.45,65.69C36.18,65.69,31,60,31,53s5-12.74,11.43-12.74S54,46,53.89,53,48.84,65.69,42.45,65.69Zm42.24,0C78.41,65.69,73.31,60,73.31,53s5-12.74,11.43-12.74S96.2,46,96.09,53,91.04,65.69,84.69,65.69Z" />
                                </svg>
                                {t('dmca.joinDiscord')}
                            </a>
                            <div className="gif-container mt-4">
                                <img src="/guidegifs/guide1.gif" alt="Guide step 1" className="guide-gif" />
                            </div>
                        </div>
                    </div>

                    {}
                    <div className="step-card animate-slide-up" style={{ animationDelay: '0.2s' }}>
                        <div className="step-number">2</div>
                        <div className="step-content">
                            <h2>{t('dmca.step2Title')}</h2>
                            <p dangerouslySetInnerHTML={{ __html: t('dmca.step2Desc') }} />
                            <div className="gif-container mt-4">
                                <img src="/guidegifs/guide2.gif" alt="Guide step 2" className="guide-gif" />
                            </div>
                        </div>
                    </div>

                    {}
                    <div className="step-card animate-slide-up" style={{ animationDelay: '0.3s' }}>
                        <div className="step-number">3</div>
                        <div className="step-content">
                            <h2>{t('dmca.step3Title')}</h2>
                            <p>{t('dmca.step3Desc')}</p>
                            <ul className="info-list">
                                <li>{t('dmca.step3Item1')}</li>
                                <li>{t('dmca.step3Item2')}</li>
                                <li>{t('dmca.step3Item3')}</li>
                            </ul>
                            <div className="icon-box">
                                <Edit3 size={24} />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </main>
    );
}
